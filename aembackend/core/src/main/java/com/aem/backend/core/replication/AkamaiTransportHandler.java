package com.aem.backend.core.replication;
 ...
 * Akamai's CCU REST APIs {@link https://api.ccu.akamai.com/ccu/v2/docs/} using
 * the replication agent properties.
 */
@Component(service = TransportHandler.class, immediate = true, name = "Custom Akamai Handler")
public class AkamaiTransportHandler implements TransportHandler {
	 ...
	 */
	private void createPostBody(final HttpPost request, final TransportContext ctx, final ReplicationTransaction tx)
			throws ReplicationException {
		...
		if (type.equals(PROPERTY_TYPE_DEFAULT)) {
			 ...
			 * 
			 * The list of activated resources (e.g.: ["/content/geometrixx/en/blog"]) is
			 * available in tx.getAction().getPaths(). For this example, we want the content
			 * created in our custom content builder which is available in
		...
		if (purgeObjects != null && purgeObjects.length() > 0) {
			try {
				json.put("type", type).put("action", action).put("domain", domain).put("objects", purgeObjects);
